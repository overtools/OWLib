...
namespace TankLib.Helpers.Hash {
    public class CRC {
        ...
        public static ulong CRC64(byte[] data, int index = 0, int length = -1, ulong crc = 0) {
            if (length == 0) return crc;
            if (length == -1) length = data.Length;
            if (index + length > data.Length) throw new IndexOutOfRangeException("CRC64 out of range");

            for (int i = 0; i != length; ++i) crc = (crc >> 8) ^ CRC64Tab[(crc ^ data[i + index]) & 0xFF];
            return ~crc;
        }
    }
}