...
namespace STUHashTool {
    ...
    public class STUFieldInfo {
        ...
        public bool Equals(STUFieldInfo obj) {
            bool firstPass = obj.hash == hash && obj.possible_array_item_size == possible_array_item_size && obj.is_nested_array == is_nested_array
                && obj.is_nested_standard == is_nested_standard && obj.possible_array == possible_array;
            if (!firstPass) {
                return false;
            }
            if (is_nested_standard || is_nested_array) {
                if (obj.nested_fields.Count != nested_fields.Count) {
                    return false;
                }
                foreach (STUFieldInfo objF in obj.nested_fields) {
    ...
    class Program {
        ...
        public static string NestedDone(STUFieldInfo[] fields, Dictionary<string, STUFieldInfo[]> doneNests) {
            uint good = 0;
            foreach (KeyValuePair<string, STUFieldInfo[]> done in doneNests) {
                if (done.Value.Length != fields.Length) {
                    continue;
                } else {
                    good = 0;
                    foreach (STUFieldInfo f1 in fields) {
                        bool found = false;
                        foreach (STUFieldInfo f2 in done.Value) {
                            if (f1.Equals(f2)) {
                                found = true;
                                good++;
                            }
                ...
                switch (fieldType) {
                    ...
                    case "nest_s":
                        ...
                        if (nestName == null) {
                            ...
                            classSb.AppendLine();
                            classSb.AppendLine(CreateInstanceClass(nestName, field.nested_fields.ToArray(), false, indentLevel + 1));
                            nestedCounter++;
                        }
