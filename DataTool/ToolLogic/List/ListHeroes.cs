    ...
    public void Parse(ICLIFlags toolFlags) {
        ...
        foreach (var (heroGuid, hero) in heroes) {
            ...
            if (!flags.Simplify) {
                ...
                Logger.Log24Bit(ConsoleColor.White.AsDOSColor().AsXTermColor().ToForeground(), null, false, Console.Out, null, $"{indent + 1}Color: {hero.GalleryColor.ToHex()} ");
                Logger.Log24Bit(hero.GalleryColor.ToForeground(), null, true, Console.Out, null, "██████");

                Logger.Log24Bit(ConsoleColor.White.AsDOSColor().AsXTermColor().ToForeground(), null, false, Console.Out, null, $"{indent + 1}sRGB Color: {hero.GalleryColor.ToNonLinear().ToHex()} ");
                Logger.Log24Bit(hero.GalleryColor.ToNonLinear().ToForeground(), null, true, Console.Out, null, "██████");
