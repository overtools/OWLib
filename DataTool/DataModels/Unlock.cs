...
public class Unlock {
    /// <summary>
    /// The hero this unlock belongs to, if any
    /// </summary>
    /// <remarks>Has to be manually set, used by the <see cref="DataTool.ToolLogic.List.ListAllUnlocks"/> mode</remarks>
    ...
    public static Unlock? Load(ulong key) {
        var stu = GetInstance<STU_3021DDED>(key);
        if (stu == null) return null;
        return new Unlock(stu, key);
    }
    ...
    private void Init(STU_3021DDED? unlock, ulong key) {
        ...
        if (unlock.m_0B1BA7C1 != null) {
            var teamDefinition = TeamDefinition.Load(unlock.m_0B1BA7C1);
