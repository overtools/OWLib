...
public class LootBox {
    ...
    public static LootBox? Load(ulong key) {
        var stu = STUHelper.GetInstance<STULootBox>(key);
        if (stu == null) return null;
        return new LootBox(stu, key);
    }

    public LootBox(STULootBox lootBox, ulong key = default) {
        Init(lootBox, key);
    }

    private void Init(STULootBox? lootBox, ulong key = default) {
