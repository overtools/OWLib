...
public class Achievement {
    ...
    public void Init(STUAchievement? achievement, ulong key = default) {
        ...
        if (achievement.m_unlock != 0) {
            Reward = Unlock.Load(achievement.m_unlock)?.ToLiteUnlock();
        }
    }

    public static Achievement? Load(ulong key) {
        var stu = GetInstance<STUAchievement>(key);
        if (stu == null) return null;
        return new Achievement(stu, key);
    }
}