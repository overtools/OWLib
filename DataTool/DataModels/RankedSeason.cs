...
public class RankedSeason {
    ...
    private void Init(STURankedSeason? season, ulong key = default) {
        ...
        YouTriedUnlocks = season.m_58066D8F?.m_unlocks?.Select(assetRef => Unlock.Load(assetRef)).ToArray();
        Top500Unlocks = season.m_heroicUnlocks?.m_unlocks?.Select(assetRef => Unlock.Load(assetRef)).ToArray();
    }

    public static RankedSeason? Load(ulong key) {
        var stu = GetInstance<STURankedSeason>(key);
        if (stu == null) return null;
        return new RankedSeason(stu, key);
    }
    ...
    public class SeasonRanks {
        ...
        public SeasonRanks(STU_A92F620A rank) {
            Min = rank.m_4BF7CD58;
            Max = rank.m_D2F14FFA;
            Unlocks = rank.m_unlocks?.m_unlocks?.Select(x => Unlock.Load(x)).ToArray();
        }
    }
}